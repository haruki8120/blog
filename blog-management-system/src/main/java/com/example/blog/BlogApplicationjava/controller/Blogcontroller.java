...
public class Blogcontroller {
  ...
  @Autowired
   private BlogRepository blogrepository;
    ...
    public String getBlogUpdateForm(@ModelAttribute BlogForm blogForm, Model model) {
        if (blogForm == null || blogForm.getId() == null) {
            throw new IllegalArgumentException("ブログIDが指定されていません。");
        }
        ...
        // BlogFormに既存のデータを設定
        blogForm.setTitle(existingBlog.getTitle());
        blogForm.setContent(existingBlog.getContent());

        // ModelにBlogオブジェクトとBlogFormを追加
